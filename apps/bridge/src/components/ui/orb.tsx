...
function Scene({
  ...
  useFrame((_, delta: number) => {
    ...
    let targetIn = 0;
    let targetOut = 0.2; // Slightly more subtle default state
    if (modeRef.current === "manual") {
      ...
      if (agentRef.current === null) {
        ...
        targetIn = clamp01(0.5 + Math.sin(t * 2.4) * 0.25);
        targetOut = 0.35;
      } else if (agentRef.current === "talking") {
        targetIn = clamp01(0.6 + Math.sin(t * 3.6) * 0.18);
        targetOut = clamp01(0.7 + Math.sin(t * 2.8) * 0.18);
      } else {
        const base = 0.3 + 0.05 * Math.sin(t * 0.5);
        const wander = 0.03 * Math.sin(t * 1.8) * Math.sin(t * 0.3 + 1.0);
        targetIn = clamp01(base + wander);
        targetOut = clamp01(0.4 + 0.1 * Math.sin(t * 0.8 + 0.5));
      }
    }
...
void main() {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        float softness = 0.8; // More diffused edges
    ...
    float ringRadius1 = sharpRing(decomposed, uTime * 0.05);
    float ringRadius2 = smoothRing(decomposed, uTime * 0.05);
    float inputRadius1 = radius + uInputVolume * 0.15;
    float inputRadius2 = radius + uInputVolume * 0.1;
    float opacity1 = mix(0.1, 0.4, uInputVolume);
    float opacity2 = mix(0.1, 0.3, uInputVolume);
